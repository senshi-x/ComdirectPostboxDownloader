...
#            print("5. (WIP) ")
    ...
    def __showStatusOnlineDocuments(self):
        ...
        self.__printFullWidth("Current Status: " + onlineStatus, "left")    
        self.__printFullWidth("--")
        self.__printLeftRight("Files online all count: ", str(self.countOnlineAll),".", 20)
        self.__printLeftRight("Files online advertisment count: ", str(len(self.onlineAdvertismentIndicesList)),".", 20)
        self.__printLeftRight("Files online not yet read count: ", str(len(self.onlineUnreadIndicesList)),".", 20)
        self.__printLeftRight("Files online in archive count: ", str(len(self.onlineArchivedIndicesList)),".", 20)
        self.__printLeftRight("Files online filename matches count: ", str(len(self.onlineFileNameMatchingIndicesList)),".", 20)
        self.__printLeftRight("Files online already downloaded count: ", str(len(self.onlineAlreadyDownloadedIndicesList)),".", 20)
        self.__printLeftRight("Files online not yet downloaded count: ", str(len(self.onlineNotYetDownloadedIndicesList)),".", 20)
        self.__printFullWidth("--")
    ...
    def __processOnlineDocuments(self, isCountRun = False):
        ...
        def __printStatus(idx, document, status=""):
            ...
            idx = idx.zfill(5)
            if not isCountRun:
                self.__printLeftRight( idx + " - " + document["dateCreation"] + " - " + document["name"] + " - " +  document["mimeType"], status, ".", menuWidth)
        ...
        downloadOnlyFromOnlineArchive = self.settings.getBoolValueForKey("downloadOnlyFromOnlineArchive")
        ...
        for idx in self.onlineDocumentsDict:
            ...
            isDocAdvertisement = True if str(documentMeta["advertisement"]).lower() == "true" else False
            isDocArchived = True if str(documentMeta["documentMetaData"]["archived"]).lower() == "true" else False
            isAlreadyRead = True if str(documentMeta["documentMetaData"]["alreadyRead"]).lower() == "true" else False
            ...
            if not bool(self.settings.getBoolValueForKey('dryRun')) and not isCountRun:
                ...
                moddate = time.mktime(datetime.datetime.strptime(documentMeta["dateCreation"],"%Y-%m-%d").timetuple())
